...
- Easy to extend into Ansible/GitOps later

## Contents

- `docs/architecture.md` — topology + responsibilities (L2 vs L3)
- `docs/decisions.md` — design tradeoffs & rationale
- `docs/writeup.md` — full write-up and deep dive
- `docs/verification.md` — how to validate the setup
- `systemd-networkd/` — config files to deploy on the host
- `libvirt/` — example VM NIC definitions (access VLAN and trunk)
...
1) Copy files into `/etc/systemd/network/` (see `scripts/install-networkd-config.sh`)
2) Restart networkd:
```bash
